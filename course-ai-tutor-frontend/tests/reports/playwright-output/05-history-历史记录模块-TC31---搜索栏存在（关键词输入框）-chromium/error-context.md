...
- Name: 05-history.spec.cjs >> 历史记录模块 >> TC31 - 搜索栏存在（关键词输入框）
- Location: tests\e2e\05-history.spec.cjs:41:3
...
Locator: locator('input[placeholder*="关键词"]').or(locator('.search-input input')).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('input[placeholder*="关键词"]').or(locator('.search-input input')).first()
